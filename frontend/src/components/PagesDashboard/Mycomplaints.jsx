...
const Mycomplaints = () => {
    ...
    const getMyRequests = async() => {
        try {
            ...
            const url = 'http://localhost:5050/GetMyComplaints';
                ...
                <div className='flex flex-col h-72 overflow-y-auto'>
                    {serverdata.map((req) => (
                        <div key={req.ComplaintID} className='flex gap-x-5 py-1.5'>
                            ...
                            <button className={`bg-red-600 rounded-md w-20 h-7 text-white font-bold flex justify-center items-center
                                                cursor-pointer hover:opacity-70 text-xs`}
                                    onClick={() => deleteComplaint(req.ComplaintID)}>Delete</button>
